...
/*  51 */   ArrayList<Planet> finished = new ArrayList<>();
    ...
    public PlanetoidChunkManager(World w, long s, boolean mapfeatures, String generatorOptions) {
        ...
        /*  86 */     //if (!generatorOptions.equals("")) Planetoid.logger.info("PlanetoidChunkManager initialized with these settings: \"" + this.generatorInfo.toString() + "\"");
    }
    ...
    @Override
    public Chunk generateChunk(int chunkX,int chunkZ){//, Block[] ablock, byte[] abyte){
        //super();
        ...
        /* 196 */     for (int i = 0; i < this.unfinished.size(); i++) {
            ...
            /* 198 */       if (p.shouldFinishChunk(chunkX, chunkZ))
                /* 199 */         p.generateChunk(chunkX, chunkZ, ablock, abyte);
            /* 200 */       if (p.isFinished()) {
        ...
        /* 210 */     for (int x = 0; x < 16; x++) {
            /* 211 */       for (int y = 0; y < 4; y++) {
                /* 212 */         for (int z = 0; z < 16; z++) {
                    /* 213 */           if(this.generatorInfo.waterFloor)Planet.setBlock(x, y, z, (y == 0) ? Blocks.BEDROCK : Blocks.WATER, 0, ablock, abyte);
                    else {Planet.setBlock(x, y, z, (y == -1) ? Blocks.BEDROCK : Blocks.AIR, 0, ablock, abyte);}
                }
            }
        }
